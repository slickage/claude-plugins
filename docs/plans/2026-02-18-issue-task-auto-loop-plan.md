# Issue-Task Auto-Loop Implementation Plan

> **For Claude:** REQUIRED SUB-SKILL: Use superpowers:executing-plans to implement this plan task-by-task.

**Goal:** Add `--auto` flag to `/issue-task` (loops all tasks with inline commits) and `/issue-start` (chains into auto-loop after planning), with `--finish`, `--on-failure`, and `--no-confirm` options.

**Architecture:** Both commands are markdown prompt files that Claude interprets. The auto-loop is implemented as conditional branching in the prompt instructions — when `--auto` is detected in `$ARGUMENTS`, the prompt switches to loop mode with inlined commit logic. No scripts or code files; all changes are to `.md` command files.

**Tech Stack:** Claude Code plugin commands (markdown with YAML frontmatter), Beads CLI (`bd`), git, `gh` CLI.

---

### Task 1: Add flag parsing and auto-loop to `issue-task.md`

**Files:**
- Modify: `plugins/issue-lifecycle/commands/issue-task.md` (full rewrite of instructions section)

**Step 1: Read the current file**

Read `plugins/issue-lifecycle/commands/issue-task.md` to confirm it matches what we expect (103 lines, 6-step manual flow).

**Step 2: Rewrite `issue-task.md` with auto-loop support**

Replace the entire file with the new version below. Key changes:
- New Step 1 parses flags (`--auto`, `--finish`, `--on-failure=stop|skip`) from `$ARGUMENTS`
- Steps 2-3 (determine issue ID, read plan) are unchanged
- Steps 4-6 (find task, implement, present) are wrapped in a conditional:
  - **Manual mode** (no `--auto`): existing single-task behavior, no commit
  - **Auto mode** (`--auto`): loop through all tasks, inline commit after each, progress summaries, final summary
- Inlined commit logic follows the same rules as `commit.md` (Conventional Commits, safe staging, HEREDOC, `bd close`)

Write the following to `plugins/issue-lifecycle/commands/issue-task.md`:
...
Use `mcp__plugin_linear_linear__create_comment` with the same format as `/issue-finish` Step 7.
...
Read the newly written file to confirm it saved correctly and the markdown is well-formed. Spot-check:
- YAML frontmatter has updated `argument-hint`
- Context section is unchanged
- Manual mode steps 4M-7M match the original steps 4-6
- Auto mode loop is complete with all steps 4A-10A
- Inlined commit logic in 7A matches `commit.md` rules

**Step 4: Commit**

```bash
git add plugins/issue-lifecycle/commands/issue-task.md
git commit -m "feat(issue-lifecycle): add --auto loop mode to issue-task command"
```

---

### Task 2: Add `--auto` and `--no-confirm` flags to `issue-start.md`
...
Read `plugins/issue-lifecycle/commands/issue-start.md` to confirm it matches what we expect (170 lines, 9-step flow).

**Step 2: Modify `issue-start.md`**

Make these targeted changes (not a full rewrite — the file is mostly unchanged):

**Change 1 — Update YAML frontmatter:**
...
**Change 2 — Add flag parsing step:**

Insert a new step between "Instructions" header and Step 1. This becomes the new Step 1, and all subsequent steps renumber:
...
**Change 3 — Modify the plan approval step (old Step 4, now Step 5):**

After the existing `AskUserQuestion` logic, add a conditional for `--no-confirm`:

```markdown
**If `--no-confirm` flag is set (requires `--auto`):** Skip this approval step entirely — auto-approve the plan and continue to Step 6.
```

Add this before the existing "If the user selects" conditionals.

**Change 4 — Modify the final summary step (old Step 9, now Step 10):**

After the existing summary output, add a conditional for `--auto`:

```markdown
**If `--auto` flag is set:** Do NOT show "Next: Run /issue-task" — instead, immediately continue into auto-loop mode. Execute the full auto-loop as defined in `/issue-task --auto --finish` — implement all unblocked tasks continuously with inline commits, then finish the issue (push, PR, Linear update). Follow all auto-loop rules from the `/issue-task` command: progress summaries after each task, failure handling (default `--on-failure=stop`), and final summary.
```

**Step 3: Verify the changes**

Read the modified file. Confirm:
- Frontmatter `argument-hint` is updated
- New Step 1 for flag parsing exists
- Plan approval step has `--no-confirm` conditional
- Final step has `--auto` chain-into-loop conditional
- Step numbering is consistent (1-10 instead of 1-9)

**Step 4: Commit**

```bash
git add plugins/issue-lifecycle/commands/issue-start.md
git commit -m "feat(issue-lifecycle): add --auto and --no-confirm flags to issue-start command"
```

---

### Task 3: Update plugin description and documentation

**Files:**
- Modify: `plugins/issue-lifecycle/.claude-plugin/plugin.json` (update description if needed)
- Modify: `docs/plans/2026-02-18-issue-task-auto-loop-design.md` (mark as implemented)

**Step 1: Read plugin.json**

Read `plugins/issue-lifecycle/.claude-plugin/plugin.json`.

**Step 2: Update design doc status**

Add a line at the top of the design doc: `**Status:** Implemented`

**Step 3: Commit**

```bash
git add plugins/issue-lifecycle/.claude-plugin/plugin.json docs/plans/2026-02-18-issue-task-auto-loop-design.md
git commit -m "docs(issue-lifecycle): mark auto-loop design as implemented"
```
...
**Step 1: Verify command files parse correctly**

Read both modified command files end-to-end:
- `plugins/issue-lifecycle/commands/issue-task.md`
- `plugins/issue-lifecycle/commands/issue-start.md`

Confirm:
- YAML frontmatter is valid
- All shell context commands (`!` prefixed) are intact
- Step numbering is sequential with no gaps
- Manual mode behavior is identical to the original for `/issue-task` without `--auto`
- Auto-loop mode references are self-contained (no "see other command" cross-references in the loop)
- Inlined commit logic matches `commit.md` safety rules (no `git add -A`, sensitive file exclusions, HEREDOC format)
- `--finish` logic mirrors `issue-finish.md` steps

**Step 2: Verify no regressions in existing commands**

Read `commit.md` and `issue-finish.md` — confirm they are unchanged.
